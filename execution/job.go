...
func NewJobFromNative(ID int, Prefix []byte, nativeMsg *evmcoretypes.Message, parentApiRouter eucommon.EthApiRouter) *Job {
	job := &Job{
		ID:        ID,
...
func (this *Job) Run(coinbase [20]byte, snapshotUrl ccurlinterfaces.Datastore) *Result { //
	...
	statedb.Prepare(this.TxHash, [32]byte{}, this.ID) // tx hash , block hash and tx index
	...
	if prechkErr == nil && this.EvmResult != nil && this.EvmResult.Err == nil && this.ApiRouter.GetReserved() != nil {
		...
		if this.EvmMsg.Gas()-this.EvmResult.UsedGas >= deferred.Native.Gas() {
			eu.VM().Context.Transfer(
				eu.VM().StateDB,
				this.EvmMsg.From(),
				eucommon.ATOMIC_HANDLER,
				big.NewInt(int64(deferred.Native.Gas())),
			)
		}
	}
